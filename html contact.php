<!DOCTYPE html>
<html lang="fr">
<head>
    <?php include 'partials/head.php'; ?>
    <link rel="stylesheet" href="header.css">
    <link rel="stylesheet" href="htmlcontact.css">
</head>
<body>

<?php include 'partials/header.php'; ?>

<h1 id="titremain">Où sommes nous</h1>
<p>nous nous trouvons dans plusieurs sites dans le monde</p>
<img src="img/map.png" class="carte-centree" />

<section class="contact-section">
    <div class="all">
        
        <div class="container">
            <h3>Liens</h3>
            <ul class="social-list">
                <li>
                    <a href="https://www.facebook.com/" target="_blank">
                        <img src="img/facebook.jpg" alt="facebook" class="social-icon">
                    </a>
                </li>
                <li>
                    <a href="https://www.instagram.com/" target="_blank">
                        <img src="img/instagram.jpg" alt="instagram" class="social-icon">
                    </a>
                </li>
                <li>
                    <a href="https://www.x.com/" target="_blank">
                        <img src="img/x.jpg" alt="x" class="social-icon">
                    </a>
                </li>
                <li>
                    <a href="https://www.linkedin.com/" target="_blank">
                        <img src="img/linkdin.jpg" alt="linkdin" class="social-icon">
                    </a>
                </li>
            </ul>
        </div>

        <div class="container">
            <div class="tel-row">
                <img src="img/telephone.png" alt="telephone" class="tel-icon">
                <h3>Téléphone</h3>
            </div>
            <p>+33 6 01 02 03 04</p>
        </div>

        <div class="container">
            <div class="adresse-row">
                <img src="img/adresse.webp" alt="adresse" class="adresse-icon">
                <h3>Adresse</h3>
            </div>
            <p>23, rue de paris<br>75002, Paris</p>
        </div>

    </div>

    <div class="barre-horizontale"></div>

    <h2 class="titre-form">Nous contacter</h2>
    <p>Vous avez des questions, des suggestions ou besoin d'assistance ? N'hésitez pas à nous contacter en remplissant le formulaire ci-dessous. Nous sommes là pour vous aider et répondre à toutes vos demandes.</p>

    <form>
        <div class="row-nom-prenom">
            <div>
                <figcaption>Nom de Famille</figcaption>
                <input type="text" name="nom">
            </div>
            <div>
                <figcaption>Prénom</figcaption>
                <input type="text" name="prenom">
            </div>
        </div>

        <figcaption>E-mail</figcaption>
        <input type="email" name="email">

        <figcaption>Votre message</figcaption>
        <textarea name="message" placeholder="écrivez ce que vous voulez" class="textarea-auto" rows="3"></textarea>

        <button type="submit">Envoyer</button>
    </form>
</section>

<?php include 'partials/footer.php'; ?>

</body>
</html>
